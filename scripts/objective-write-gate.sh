...
#      - the sanctioned Write to THIS session's objective file -> Rule 2 validation
...
case "$TOOL" in
    ...
    if [ -n "$TGT" ]; then
      ...
      if under_home "$RTGT"; then
        if [ "$RTGT" = "$RFILE" ] && [ "$TOOL" = "Write" ]; then
          ...
          so_deny_pretooluse "session-objective: $TOOL on $TGT is denied. The objective home ($(so_home)) is hook-written. This session's objective file is $FILE and the ONLY sanctioned change to it is the Write tool with file_path exactly that path, rewriting the whole file with the OPERATOR LEDGER layer byte-for-byte unchanged."
        fi
      else
        RTGT=""
      fi
  ...
  Bash)
    ...
    if grep -qF -- "$(so_home)" <<< "$CMD" || grep -qF -- "$HOMEDIR" <<< "$CMD"; then
      ...
      so_deny_pretooluse "session-objective: this Bash command names the objective home ($(so_home)) and is denied. The OPERATOR LEDGER layer is append-only and hook-written; the OBJECTIVE layer is changed only by the Write tool with file_path exactly $FILE. Use: Write, file_path=$FILE. Command refused: $CMD"
    fi
    RTGT=""
    ;;
  *) RTGT="" ;;
...
BOUND="$(so_bound "$FILE")"
if [ -z "$BOUND" ]; then
  so_deny_pretooluse "session-objective: $FILE has no readable 'bound to ledger entry <number>' heading, so the write-before-act lock cannot release. Rewrite the whole file with the Write tool, file_path exactly $FILE, heading: # OBJECTIVE (agent-written, rewritten every turn, revision 1, bound to ledger entry $COUNT)"
fi
if [ "$BOUND" != "$COUNT" ]; then
  so_deny_pretooluse "session-objective: $TOOL is denied — the objective is bound to ledger entry $BOUND and the operator's latest message is entry $COUNT. Rewrite the objective FIRST, then work. Exactly one tool call is permitted right now: Write, with file_path=$FILE, rewriting the whole file, OPERATOR LEDGER layer unchanged, heading bound to ledger entry $COUNT. No other tool, no Bash heredoc, no Edit. If the Write tool refuses because the file changed on disk, write it again: the copy in the SESSION OBJECTIVE block in your context IS the current file, so you do not need to read it first."
fi
exit 0
